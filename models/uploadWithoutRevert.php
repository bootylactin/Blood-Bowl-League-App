...
class BbqlModelUpload extends JModel
{
	...
	function uploadMatchReport() {
		...
		if(move_uploaded_file($_FILES['userfile']['tmp_name'],$this->rel_processing_path.$this->filename)) {
			...
			//deterimine if the app is fully controlling this league
			...
			$sql = "UPDATE Statistics_Season_Teams SET iMatchPlayed = iMatchPlayed + 1,";
			foreach($statFields as $value) {
				$sql = $sql.$value." = ".$value."+".$matchStats['Away_'.$value] . ",";
			}
			$sql = $sql."iPoints = iPoints + ".$awayPoints.
				",iWins = iWins + ".$awayWins.
				",iDraws = iDraws + ".$awayDraws.
				",iLoss = iLoss + ".$awayLosses.
				",touchdownDif = touchdownDif + ".$awayTeamScore." - ".$homeTeamScore;
				if (!$fullControl) {
					$sql = $sql.",iSpectators = iSpectators + ".$matchStats['iSpectators'];
				}
				$sql = $sql.",Average_iMatchRating = Average_iMatchRating + ".$matchStats['iRating'];
			...
			$sql = "UPDATE Statistics_Season_Teams SET iMatchPlayed = iMatchPlayed + 1,";
			foreach($statFields as $value) {
				$sql = $sql.$value." = ".$value."+".$matchStats['Home_'.$value] . ",";
			}
			$sql = $sql."iPoints = iPoints + ".$homePoints.
				",iWins = iWins + ".$homeWins.
				",iDraws = iDraws + ".$homeDraws.
				",iLoss = iLoss + ".$homeLosses.
				",touchdownDif = touchdownDif + ".$homeTeamScore." - ".$awayTeamScore;
				if (!$fullControl) {
					$sql = $sql.",iSpectators = iSpectators + ".$matchStats['iSpectators'];
				}
				$sql = $sql.",Average_iMatchRating = Average_iMatchRating + ".$matchStats['iRating'];
			...
			foreach($playerStats as $statsRow) {
				...
				foreach($playerStatFields as $fieldsRow) {
					$sql = $sql.$fieldsRow." = ".$fieldsRow."+".$statsRow[$fieldsRow] . ",";
					//calculate SPP
					switch ($fieldsRow) {
						case 'iMVP':
			...
			if ($fullControl) {
				...
				if ($awayWins == 0) {
					$sql = $sql."iCash = iCash + ".$Away_iCashEarned.",";
				...
				//commit money immediately if team drew or lost
				if ($homeWins == 0) {
					$sql = $sql."iCash = iCash + ".$Home_iCashEarned.",";
    ...
    function uploadBBxxxLog() {
    	//TODO $txt = file_get_contents();
		$txt = $this->httpPathToComponent.DS.'uploads'.DS.'19'.DS.'266.log';
		var_dump($txt);
    }
