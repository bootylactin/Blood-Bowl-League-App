...
class BbqlModelUpload extends JModel
{
	...
	function __construct() {
		...
		//var_dump(filesize($_FILES['userfile']['tmp_name']));
		//die();
	...
	function uploadTeamFile($teamType, $password, $fullControl) {
		...
		if(move_uploaded_file($_FILES['userfile']['tmp_name'],$this->rel_processing_path.$this->filename)) {
			...
			if ($dbHandle->query("select * from player_listing") == false)
				return -4;
			...
			'idStrings_Formatted_Background','idStrings_Localized_Leitmotiv','iNextPurchase');
	...
	function uploadMatchReport($revert=false) {
		...
		if(move_uploaded_file($_FILES['userfile']['tmp_name'],$this->rel_processing_path.$this->filename)) {
			//first we have to make the .db format accessible to php
			var_dump($this->rel_processing_path.$this->filename);
			$this->convertDBFile($this->filename);
			...
			if ($matchResultHandle->query("select * from player_listing") == false)
				return array("result" => "error", "status" => -4);
